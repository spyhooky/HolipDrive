/** @file */  /* DoxyGen file declaration */
/** @defgroup MOC_Communication MOC Communication
 *  @inlayer service
 *  @{
 */
//---------------------------------------------------------------------------
//  $Header$
//
//  Company    : Danfoss Drives A/S
//
//  Project    : P618 HVAC Basic (AOC)
//
//  Filename   : Crc8.c
//
//  Programmer : Han Feng, PE/BJ
//
//  Function   :
//
//  Entries    :
//
//
//
//                      ***  Confidential property of Danfoss Drives A/S ***
//                               Copyright(c) DD-MC, Danfoss Drives A/S, 2012
//---------------------------------------------------------------------------

//-------------------- pragmas ----------------------------------------------
//-------------------- include files ----------------------------------------
#include "Holipheader.h"
#include "Crc8.h"

//-------------------- local definitions ------------------------------------
//-------------------- public data ------------------------------------------
//-------------------- private data -----------------------------------------
//---------------------------------------------------------------------------
// Select 0xA6 = x^8 +x^6 +x^3 +x^2 +1 (0xB2 for LSB-first code) 
// as crc8 polynomial,http://www.ece.cmu.edu/~koopman/crc/0xa6.txt
// LSB-first code definition can be found at 
// http://en.wikipedia.org/wiki/Cyclic_redundancy_check.
// Below table is generated according to 0xB2.
static const UNSIGNED8 abyCrc8Table[256] = {
   ,
};

//-------------------- private typedef --------------------------------------
//-------------------- private function prototypes --------------------------
//-------------------- private functions ------------------------------------
//-------------------- public functions -------------------------------------
//---------------------------------------------------------------------------
//  Name     :        byCalCrc8 
//                  ======================================
//  Function :  calculate a crc8 over the given input data
//          
//  Input    : - pbyData:  the address of data buffer
//             - lDataLen: the length of data buffer in bytes
//             - byCrc8:   the init crc8 value as a "seed"
//  Output   : crc8 checksum
//---------------------------------------------------------------------------
UNSIGNED8 Crc8_byCalCrc8(UNSIGNED8 *pbyData, UNSIGNED32 dwDataLen, UNSIGNED8 byCrc8)
{
   while(dwDataLen-- > 0)
   {
      byCrc8 = abyCrc8Table[byCrc8 ^ *(pbyData++)];
   }
   return byCrc8;
}

/** @} *//* End of group */
//---------------------------------------------------------------------------
//  End of file
//---------------------------------------------------------------------------
